...
//category selectory onclick
categorySelector.addEventListener('change',()=>{
    const selectedCategory = categorySelector.value;
    popActivitySelector(selectedCategory);
    activityTable.innerHTML ='';
    if(selectedCategory === selectOne){
        console.log('selectOne',selectOne.value)
        alert('You have selected a empty category');
    }else popActivitySelector(selectedCategory);
})

  //activity selector onclick
  activitySelector.addEventListener('change',()=>{
    const selectedActivityId = activitySelector.value;
    displayActivityDetails(selectedActivityId);
  });

  //fuctions
 function  popActivitySelector(category){
 ...
 function displayActivityDetails(activityId){
  ...
  const hCell1 = hRow.insertCell();
  hCell1.textContent = 'Name';
  const hCell2 = hRow.insertCell();
  hCell2.textContent = 'Price';
  const hCell3 = hRow.insertCell();
  hCell3.textContent = 'Location';

  // Create row
  const row1 = table.insertRow();
  const Cell1 = row1.insertCell();
  Cell1.textContent = selectedActivity.name;
  const Cell2 = row1.insertCell();
  Cell2.textContent = '$' + selectedActivity.price;
  const Cell3 = row1.insertCell();
  Cell3.textContent = selectedActivity.location;
